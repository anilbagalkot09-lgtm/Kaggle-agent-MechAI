...
def average_daily(forecast):
    arr = forecast.get('forecast', [])
...
class ReorderAgent:
    ...
    async def on_message(self, msg: A2AMessage):
        ...
        ad = average_daily(forecast)
        reorder_point = math.ceil((sku_rec.get('lead_time_days',7) * ad) + 2)  # simple safety stock
